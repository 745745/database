...
bool Record::get(iovec *iov, int iovcnt, unsigned char *header)
{
    ...
    // 最后一个字段长度
    int last = length - vec[iovcnt - 1] - offset;
    vec[iovcnt - 1] =last ;
...
iovec *Record::getKey(int key)
{
    ...
    int ptr = it.size();
    for (; it.get() != 1; cnt++) {
        it.decode((char *) buffer_ + ptr, 8);
        offset.push_back(it.get());
        ptr += it.size();
    }
    //ptr += 1; // header size

    int iov_len;
    if (key == offset.size()-1)
        iov_len = len - offset[0] - ptr;
    else
        iov_len = offset[offset.size() - key-2] - offset[offset.size() - key-1];

    iovec *a = new iovec;
    a->iov_base = (char *) buffer_ + ptr + offset[offset.size() - key-1];
    a->iov_len = iov_len;
    return a;
}

bool Record::is_delete() 
{
    ...
    offset += 1;
    memcpy(&header, buffer_ + offset, 1);
    if ((header & MASK_DELETE_TRUE) == 0)
        return false;
    else
        return true;
...
bool Record::IsOverBlock() 
{
    ...
    offset += 1;
    memcpy(&header, buffer_ + offset, 1);
    if ((header & MASK_OVER_BLOCK_TRUE) == 0)
        return false;
    else
        return true;
...
void Record::setheader(unsigned char header) 
{ 
    int offset=0;
    Integer it;
    it.decode((char*)buffer_, 8);
    ...
    memcpy(buffer_ + offset, &header, 1);
}
