...
class Block
{
  ...
  public:
    ...
    //清理已删除的记录
    void clearGarbage(File& x);

	//key是否比整个block的key都大，若是返回true,不是返回false
	bool LargerThanBlock(iovec* key);

	//获取所有记录
	void getAllRecord(std::vector<iovec*>&allRecord);

	////
	//参数:要插入的记录
	//作用:判断能否插入
	//返回值:能插入=true,不能为false
	////
...
class DataBlock : public Block
{
  public:
    //清内存
    void clear(unsigned int blockid, unsigned int spaceid);
    ...
    ////
    //参数:key=主键，type=主键的数据类型,fileoffset=要写在文件的位置,x=文件
    //作用:Block内slot排序,完成后会将整个block刷写到硬盘上
    //返回值:无
    ////
    void BlockSort(int key, char *type,int fileoffset,File& x);
