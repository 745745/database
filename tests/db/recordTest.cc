...
TEST_CASE("db/record.h")
{
    SECTION("size")
    {
        ...
        bret = record.ref(iov2, 4, &header2);
        REQUIRE(bret);
        REQUIRE(strncmp(b1, table, strlen(table)) == 0);
        REQUIRE(iov2[0].iov_len == strlen(table) + 1);
        REQUIRE(type2 == type);
    ...
    { 
        ...
        size_t sret = rd.set(iov, 4, &header);
        REQUIRE(header == (unsigned char)0xF0);

        REQUIRE(!rd.is_delete());
        REQUIRE(rd.IsOverBlock());

        header = Record::initHeader(false, true);
        REQUIRE(header == (unsigned char) 0x0F);
        rd.setheader(header);
        REQUIRE(rd.is_delete());
        REQUIRE(!rd.IsOverBlock());
    ...
    SECTION("getKey") 
    {
        ...
        iovec *a = record.getKey(2);
        char *check = (char*)malloc(a->iov_len);
        memcpy(check, a->iov_base, a->iov_len);
        std::string x=check;
        std::string b=hello;
        REQUIRE(x==b);
